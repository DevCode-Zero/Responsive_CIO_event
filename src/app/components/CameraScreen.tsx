...
export function CameraScreen({ onSuccess }: CameraScreenProps) {
  ...
  const recognizeFace = async () => {
    ...
    try {
      ...
      if (best.name === "unknown") {
        setStatus("error");
        setErrorMessage("Oops! this is funny, we couldn't recognize you. Please contact your admin.");
      ...
      if (matched) {
        ...
        await db.checkInAttendee(matched.id);
        const updatedAttendee = { ...matched, checked_in_at: new Date().toISOString() };
